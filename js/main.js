    ...
    dataReady = function () {//This is called as soon as data is loaded
        ...
        a.iterNodes(
            function (b) { //This is where we populate the array used for the group select box
                b.group = b.attr.attributes.group;
                // note: index may not be consistent for all nodes. Should calculate each time. 
                // alert(JSON.stringify(b.attr.attributes[5].val));
                // alert(b.x);
                a.clusters[b.group] || (a.clusters[b.group] = []);
                a.clusters[b.group].push(b.id);//SAH: push id not label
                a.colors[b.group] = b.color;
            }
...
function configSigmaElements(config) {
    ...
    for (b in sigInst.clusters) {
        var c = sigInst.colors[b];
        a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + c + ';display:inline-block"></div> <div style="display:inline-block">' + b + '</div> (' + sigInst.clusters[b].length + ' members)</a></div>');
    }
    //a.sort();
